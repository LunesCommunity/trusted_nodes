...
nblocks = 40080 # roughly one month
total_balance = 0
total_fees = 0
generators = []
unique_generators = []
...
blacklist = ['']
data = {'node': []}
trusted_json_path = '/var/www/lunescommunity/trusted.json'
trusted_json_path2 = '/var/www/LUNES/stats/trusted.json' #only for lunesfullnode.com
if len(sys.argv) > 1:
...
for generator in set([x[0] for x in generators]):
    fees = sum(g[1] for g in [x for x in generators if x[0] == generator])
    count = sum(1 for g in [x for x in generators if x[0] == generator])
    total_fees += fees
...
with open(trusted_json_path2, 'w') as outfile: #only for lunesfullnode.com 
    json.dump(data, outfile)
