...
class Address(object):
    def __init__(self, address='', publicKey='', privateKey='', seed='', alias='', nonce=0):
        ...
        if seed:
            ...
            self.address = pywaves.wrapper('/addresses/alias/by-alias/%s' % alias).get("address", "")
            self.publicKey = ''
            self.privateKey = ''
            self.seed = ''
            self.nonce = 0
    ...
    def aliases(self):
        a = pywaves.wrapper('/addresses/alias/by-address/%s' % self.address)
        if type(a)==list:
    ...
    def createAlias(self, alias, txFee=pywaves.DEFAULT_ALIAS_FEE, timestamp=0):
        ...
        if not self.privateKey:
            ...
            return pywaves.wrapper('/addresses/alias/broadcast/create', data)
